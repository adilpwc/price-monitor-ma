...
class HtmlScraper(BaseScraper):
    ...
    async def search(self, product: ProductConfig) -> list[Offer]:
        ...
        try:
            response = await self.client.get(
                self.site.build_search_url(),
                params={self.site.query_param: product.name},
            )
            ...
            if status == 403:
                raise ScraperError(
                    f"{self.site.name}: HTTP 403 accès automatisé refusé "
                    f"durée={duration:.2f}s url={url}; aucun contournement tenté"
                ) from exc
            raise ScraperError(
                f"{self.site.name}: HTTP {status} durée={duration:.2f}s url={url}"
            ) from exc
        except httpx.RequestError as exc:
            duration = perf_counter() - started
            raise ScraperError(
                f"{self.site.name}: erreur réseau {type(exc).__name__} "
                f"durée={duration:.2f}s url={_safe_log(exc.request.url)}"
            ) from exc
    ...
    def parse(self, html: str, product: ProductConfig) -> list[Offer]:
        ...
        result = sorted(
            unique.values(),
            key=lambda offer: (-offer.match_score, offer.price),
        )
        LOGGER.info(
            "Analyse HTML site=%s cartes=%d offres_cartes=%d "
            "offres_json_ld=%d offres_uniques=%d",
            self.site.name,
            cards_detected,
            len(card_offers),
            len(json_ld_offers),
    ...
    def _product_url(self, value: str) -> str | None:
        ...
        if parsed.scheme != "https" or host != base_host:
            LOGGER.debug(
                "URL produit rejetée site=%s raison=domaine url=%s",
                self.site.name,
                _safe_log(candidate),
            )
            return None
        ...
        if self.site.product_url_include and not any(
            token in path_and_query for token in self.site.product_url_include
        ):
            return None
    ...
    def _parse_cards(
        ...
        for card in soup.select(self.site.card_selector):
            ...
            if self.site.availability_selector:
                ...
                content = (
                    availability.get_text(" ", strip=True).lower()
                    if availability
                    else ""
                )
            offers.append(
                Offer(
                    ...
                    currency="MAD",
                    available=not any(
                        token in content for token in self.site.unavailable_text
                    ),
                    url=url,
                    image_url=image_url,
    ...
    def _price_from_node(node: Tag) -> Decimal:
        ...
        value = content if isinstance(content, str) else node.get_text(" ", strip=True)
            ...
            for item in self._objects(payload):
                ...
                for offer_data in offer_values:
                    if not isinstance(offer_data, dict):
                        continue
                    price_value = offer_data.get(
                        "price",
                        offer_data.get("lowPrice"),
                    )
                    raw_url = str(
                        item.get("url") or offer_data.get("url") or ""
                    ).strip()
                    ...
                    try:
                        ...
                        continue
                    availability = str(
                        offer_data.get("availability", "InStock")
                    ).lower()
                    offers.append(
                        Offer(
                            query=product.name,
    ...
    def _is_product(value: object) -> bool:
        ...
        return any(
            str(item).rstrip("/").rsplit("/", 1)[-1] == "Product" for item in values
        )
